...
def main():
    ...
    for k, (tag, z) in enumerate(POINTS):
        ...
        ax.plot(mu_grid, p_mu, color="#2ca02c", lw=2.2, label="flow (slope-reg)")
        # benchmark power-law guide, anchored at mu=3
        i0 = np.argmin(np.abs(mu_grid - 3.0))
        guide = p_mu[i0] * (mu_grid / 3.0) ** (-ALPHA)
        m = mu_grid >= 3.0
        ax.plot(mu_grid[m], guide[m], "k--", lw=1.2, alpha=0.8, label=fr"$\mu^{{-{ALPHA}}}$ guide")
        ax.set_xscale("log"); ax.set_yscale("log"); ax.set_ylim(1e-3, None); ax.set_xlim(0.6, 12)
        ax.set_title(f"{tag}"); ax.set_xlabel(r"$\mu$"); ax.set_ylabel(r"$dP/d\mu$")
        ax.grid(True, ls=":", alpha=0.4)
        if k == 0:
            ax.legend(fontsize=9)
    fig.suptitle(r"Tail check: simulator vs slope-regularized flow ($h=0.72,\Omega_M=0.38,\sigma_8=1.00$)")
    fig.tight_layout()
