...
def bootstrap_alpha(xi_evt, xi_bins_log, xi_centers, xmin, xmax, n_bootstrap, seed):
    ...
    for _ in range(n_bootstrap):
        ...
        fit = fit_power_law_window(xi_centers, dens, xmin, xmax)
...
def measure_alpha_for_run(gw, *, z, sigma8, omega_m, h, nreal, seed, nhalos, xmin, xmax, n_bootstrap):
    ...
    xi_centers, xi_density, _ = hist_density(xi_evt, xi_bins_log)
    ...
    for x0, x1 in [
        ...
        fit = fit_power_law_window(xi_centers, xi_density, x0, x1)
        if fit is not None:
            window_scan.append(fit)

    bootstrap = bootstrap_alpha(xi_evt, xi_bins_log, xi_centers, xmin, xmax, n_bootstrap, seed + 17) if n_bootstrap > 0 else None
    ...
    out = {
        ...
        "Nreal": int(nreal),
        "Nhalos": int(nhalos),
        "alpha": float(fit_window["alpha"]) if fit_window is not None else None,
        "alpha_bootstrap_std": None if bootstrap is None else bootstrap["std"],
        "alpha_p16": None if bootstrap is None else bootstrap["p16"],
        "alpha_p50": None if bootstrap is None else bootstrap["p50"],
        "alpha_p84": None if bootstrap is None else bootstrap["p84"],
...
def plot_alpha_vs_z(rows, out_path):
    ...
    for sigma8 in sigma8_list:
        subset = sorted([r for r in rows if r["sigma8"] == sigma8], key=lambda r: r["z"])
    ...
    ax.set_xlabel("z")
    ax.set_ylabel("alpha")
    ax.set_title("Tail exponent alpha vs redshift")
...
def plot_alpha_vs_sigma8(rows, out_path):
    ...
    fig, ax = plt.subplots(figsize=(7.0, 4.8))
    for z in z_list:
        subset = sorted([r for r in rows if r["z"] == z], key=lambda r: r["sigma8"])
    ...
    ax.set_ylabel("alpha")
    ax.set_title("Tail exponent alpha vs sigma8")
    ax.legend(frameon=False, fontsize=8)
    fig.tight_layout()
    fig.savefig(out_path, dpi=170)
...
def plot_alpha_histogram(rows, out_path):
    alpha = np.array([r["alpha_p50"] if r["alpha_p50"] is not None else r["alpha"] for r in rows], dtype=float)
    fig, ax = plt.subplots(figsize=(6.4, 4.8))
    ax.hist(alpha, bins=min(8, max(4, alpha.size // 2)), edgecolor="black")
    ax.set_xlabel("alpha")
    ax.set_ylabel("count")
    ax.set_title("Distribution of measured alpha values")
    fig.tight_layout()
    fig.savefig(out_path, dpi=170)
...
def main():
    parser = argparse.ArgumentParser(
        description="Measure the tail exponent alpha(z, sigma8) for the event-level jump measure on the Track A grid."
    )
    ...
    parser.add_argument("--out-prefix", default="alpha_grid")
        ...
        for sigma8 in sigma8_list:
            rows.append(
                measure_alpha_for_run(
                    ...
                    seed=args.seed,
                    nhalos=args.Nhalos,
                    xmin=args.power_xmin,
                    xmax=args.power_xmax,
    ...
    print(
        f"WROTE {out_prefix}.csv {out_prefix}.json "
        f"{out_prefix}_vs_z.png {out_prefix}_vs_sigma8.png {out_prefix}_histogram.png"
    )
