...
scanned M = 1e5 .. 1e20 Msun (one per decade; r200 alternative axis =
...
Arms per z_s in {1, 5}: 16 field arms + anchors `legacy` (bias_model=0 iid)
...
ZS_LIST = [1.0, 5.0]
LOGM_LIST = list(range(5, 21))                # 1e5 .. 1e20, one per decade
...
def stage_report(args):
    ...
    for iz, zs in enumerate(ZS_LIST):
        ...
        ax.set_title(f"$z_s={zs:g}$: field arms, $M_{{clust}}=10^5..10^{{20}}$")
...
def main():
    ...
    ap.add_argument("stage", choices=["mc", "report", "shard"])
    ...
    args = ap.parse_args()
    {"mc": stage_mc, "report": stage_report, "shard": stage_shard}[args.stage](args)
