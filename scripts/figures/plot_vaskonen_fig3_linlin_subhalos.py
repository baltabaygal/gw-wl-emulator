"""
Linear-linear Vaskonen Fig. 3 style redshift scatter plot, with subhalos.

The plotted quantity is std(1/sqrt(mu)) = sigma_D_L / D_L from Monte Carlo
ln(mu) samples. Results are cached so exploratory runs can be refined with
larger --nreal values without repeating completed work.
"""
...
def sigma_dl_from_lnmu(lnmu: np.ndarray) -> tuple[float, float, int]:
    """std(D_L/D_L,bg) in the SOURCE plane, matching Vaskonen's main_lensing.

    The raw MC samples from sample_lnmu are image-plane (one lnmu per random line
    of sight). The observable magnification PDF for sources is the source-plane one,
    P_S ~ P_I / mu (halos/lensing.cpp Plnmuf: "convert from image plane to source
    plane"). We reproduce that by weighting each sample by w = 1/mu = exp(-lnmu)
    instead of binning->resampling; the two are equal in expectation. Variance is
    population (ddof=0), matching the C++ variance() in basics.cpp. Skipping this
    reweight inflates sigma_DL/D_L by ~10-12%.
    """
    valid = np.asarray(lnmu, dtype=float)
    valid = valid[np.isfinite(valid)]
    valid = valid[valid >= -2.302585]  # Filter out unphysical demagnified strong-lensing outliers (mu < 0.1)
    dl_factor = np.exp(-0.5 * valid)               # D_L / D_L,bg = 1/sqrt(mu)
    w = np.exp(-valid)                             # source-plane weight 1/mu
    wsum = float(np.sum(w))
    mean = float(np.sum(w * dl_factor) / wsum)
    var = float(np.sum(w * (dl_factor - mean) ** 2) / wsum)  # population (ddof=0)
    sigma = float(np.sqrt(max(0.0, var)))
    n_eff = wsum * wsum / float(np.sum(w * w))     # Kish effective sample size
...
def run_simulation_task(task: tuple) -> tuple[int, int, float, float, int]:
    ...
    sigma, err, n = sigma_dl_from_lnmu(lnmu)  # shared source-plane helper (single source of truth)
...
def run_curve(z_values: np.ndarray, curve: dict, args: argparse.Namespace) -> dict:
    ...
    for i, z in enumerate(z_values):
        ...
        print(f"{curve['label']}: z={z:g}, seed={seed}, nreal={args.nreal}", flush=True)
        lnmu = gw.sample_lnmu(
            float(z),
            COSMO["OmegaM"],
            COSMO["sigma8"],
...
def run_ace_curve(z_values: np.ndarray) -> dict:
    y = []
    for z in z_values:
        print(f"ACE lensing: z={z:g}", flush=True)
        try:
            y.append(sigma_dl_from_ace_pdf(float(z)))
        except ValueError as exc:
            print(f"ACE lensing: z={z:g} skipped ({exc})", flush=True)
            y.append(float("nan"))
    return {
        "label": "ACE lensing",
        "color": "#bfbfbf",
...
def save_plot(results: dict, png_path: Path, pdf_path: Path, ylim: tuple[float, float] = None) -> None:
    ...
    for curve in results["curves"]:
        ...
        # Override color and linestyle from local CURVES configuration to bypass outdated values in JSON cache
        color = curve["color"]
    ...
    ax.set_xlabel(r"$z$", color="black", fontsize=14)
    ax.set_ylabel(r"$\sigma_{D_L}/D_L$", color="black", fontsize=14)
    ax.tick_params(colors="black", which="both", width=1.0, labelsize=12)
    for spine in ax.spines.values():
        spine.set_color("black")
        spine.set_linewidth(1.0)
    leg = ax.legend(loc="lower right", fontsize=10.5)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--cache",
        type=Path,
        default=ROOT / "data" / "vaskonen_fig3_linlin_subhalos.json",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "plots" / "figures" / "vaskonen_fig3_linlin_subhalos.png",
    )
...
def main() -> None:
    ...
    if args.cache.exists() and not args.force:
        ...
        if not args.no_ace and not has_ace:
            ...
            print(f"Updated cache with ACE curve: {args.cache}")
    else:
        if args.jobs > 1:
            ...
            # Sort by heaviest tasks first (subhalos=True, high redshift) to maximize CPU utilization
            tasks.sort(key=lambda t: (t[3].get("subhalo", False), t[2]), reverse=True)
            
            print(f"Running simulations in parallel across {args.jobs} processes...", flush=True)
            curves_data = {
                c_idx: {
                    "y": [0.0] * len(z_values),
                    "yerr": [0.0] * len(z_values),
                    "nvalid": [0] * len(z_values),
                }
                for c_idx in range(len(CURVES))
            }
            
            # max_tasks_per_child=1: fresh interpreter per task so gwlensing cannot
            # carry state from a previous task's configuration (corrupted the n400000 run)
