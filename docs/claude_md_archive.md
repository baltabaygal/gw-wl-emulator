...
## From "The `halos` fork" status list — items 9–13 (moved 2026-07-16 by /doctor, user-approved)

Verbatim from CLAUDE.md at the time of the move; condensed decision summaries with
pointers remain there. If any of this becomes active again, move it back.
...
13. **BIAS layer couples a physical scale to the numerical grid — strong tail
   ...
   **DEEP-TAIL FINDING (2026-07-16, `tmp/deep_tails.py`, z_s=1 window arms
   extended to 4.8M real/arm, N-arms 2.4M; survival-function figs
   `plots/deep_tail_{window,N}.png`; cache bitwise-verified before+after a
   mid-run module rebuild — 87 crashed shards refilled):** the clustering
   tail enhancement GROWS into the far tail — S_field/S_nobias at
   R_⊥=8.44 Mpc: 1.23±0.02 / 1.52±0.07 / 1.78±0.16 / 1.90±0.27 / 2.34±0.44
   at lnμ > 0.5/1/1.5/2/2.5 (10 Mpc: 1.18→1.66; 18.2 Mpc: 1.04→1.39;
   shard-jackknife). Since ⟨λ⟩=1 preserves single-encounter rates, growth =
   the deep tail is increasingly multi-cell/pair coincidences boosted by
   E[λiλj]=exp(b̃ib̃jCov)>1. Enhancement is ⟨N⟩-INDEPENDENT (matched pairs
   at ⟨N⟩=100/300/1000 overlap) — the tail lenses are already explicit at
   ⟨N⟩=100, cleanly separating mechanisms (body share ∝ ⟨N⟩, tail not).
   **REVISES "window choice is soft": soft in the BODY only** — at lnμ>1
   the 8.44 vs 18.2 Mpc arms differ by 1.52 vs 1.06 (~40% relative tail
   rate). Immaterial at O(1000) events (P(lnμ>1)~1.6e-4 → ~0.16
   events/1000) but decisive if tail-sensitive statistics are ever used.
   Standing caveat unchanged: ABSOLUTE far-tail not certified in this
   method class (needs N-body per turboGL deconstruction; lnμ≳2 is κ~O(1))
   — these are within-model sensitivities.
   ...
   **bias_weak C++ WIRING DONE (2026-07-16, supervisor approved the Cox-split
   formulation; ⚠ Mac build + gates PENDING):** new flag `bias_weak` (default
   off; requires bias_model=1, throws otherwise; no-op when bias=0) through
   the full chain (LensingConfig/SamplingParams/5 py::args/config dict).
   κ_W is drawn conditionally on the SAME realized field as the counts:
   `weakMomentsNFW` = per-cell m=∫n̄κ, v=∫n̄κ² over sub-threshold annuli
   (verbatim sigmakappaW loop semantics; Σv==σ_W² gate THROWS at build if
   violated), `BiasField1D::buildWeak` per-shell log tables T_i(δ)=Σmλ,
   V_i(δ)=Σvλ (193-pt ±6σ_i grid, ln-interp; λ = the count layer's mean-1
   lognormal, per flag (a) above), sampler κ_W = Σ S_i(δ̄_i) +
   N(0,1)√(Σ V_i(δ̄_i)), S_i=T_i−Σm (static weak mean stays in the ⟨κ⟩=0
   anchor). Adds weak 2-halo variance + weak↔count covariance free. Pre-build
   validation PASSED (validate_field_covariance.py weak layer): ΣΔv~4e-16,
   table interp errS≤3.3e-3 errV≤5.3e-4, ⟨S⟩=0, ⟨V⟩/σ_W²=1±1e-3;
   **Var(ΣS)/linearized-BCB = 1.03–1.07 (R_⊥≈84 Mpc) but 1.97–2.87 at the
   8.44 Mpc default and ~40 at 0.84 Mpc — the earlier linearized weak-arm
   sizing UNDERSTATES the full lognormal term at small R_⊥.** Module smoke
   extended (joint arm determinism, Var(joint)>Var(counts-only), guard
   throw). Scan extended: joint arms w1e07–w1e20 APPENDED (counts-only arm
   seeds unchanged — cache valid) + `wN300` (⟨N⟩=300 joint arm; report
   prints the split-invariance JSD(w1e14,wN300) vs floors — the reviewer's
   acceptance gate that moving κ_thr only shuffles mass between the two
   field-riding domains). Reviewer acceptance list (2026-07-16 sign-off):
   regression counts-only, bitwise model-0, Σv gate, split invariance,
   Cov(κ_W,N_h) structure (optional), linearized cross-check. NFW-only scope
   (filaments carry no weak arm — call out in the design note).
