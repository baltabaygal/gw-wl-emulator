...
We estimate the halo clustering using the one-dimensional density field $\delta_{\rm 1D}(z) \equiv \delta_{\rm 1D}(d_c(z))$, obtained by \B{averaging the three-dimensional density field over a transverse disk of comoving radius $R_\perp$ centered on the line of sight}. Its power spectrum is\footnote{This generalizes the pencil-beam construction of Ref.~\cite{1991ApJ...379..482K}, whose $P_{\rm 1D}(k_\parallel)=\int_{k_\parallel}^\infty \td k\,k\,P(k)/(2\pi)$ is recovered in the \B{$R_\perp\to0$} limit.}
\be
    \B{P_{\rm 1D}(k_\parallel) = \frac{1}{2\pi} \int_0^\infty \!\td k_\perp \, k_\perp \, P\big(\sqrt{k_\parallel^2+k_\perp^2}\,\big)\, \tilde{W}^2(R_\perp k_\perp) \,,}
\ee
\B{where $\tilde{W}(x) = 2J_1(x)/x$ is the Fourier transform of the disk window. The transverse averaging suppresses the contribution of scales smaller than $R_\perp$ perpendicular to the line of sight; along the line of sight, modes with $k_\parallel > 2\pi/R_\perp$ are excluded from the discrete realization. We use $R_\perp = 8.4\,$Mpc, the Lagrangian radius of $M = 10^{14}\Msun$ halos, which is the clustering-variance-weighted signal scale of the model; we show later how changing $R_\perp$ impacts our results.}
...
\ee
with $\langle \sum_i m_i \rangle = f_{\rm s} M$, so that the total halo mass is conserved \B{in expectation; the Poisson fluctuations of the substructure about this mean are part of the halo-to-halo scatter the model is designed to capture. We reduce the smooth component by the deterministic bound fraction rather than by the realized $\sum_i m_i$ because only subhalos above a resolution floor are explicitly sampled (see below); the unresolved remainder enters through its mean and variance, and the decomposition is exact in both precisely for this split.} The reduction of the smooth component is essential, since we consider that the halo mass function counts the total (substructure-inclusive) mass, so the subhalos must be carved out of the halo rather than added on top of it.
...
\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{plots/sigma_k_vs_subhalo_factor.png}
    \caption{subhalo convergence. this is placeholder plot. will be improved.}
...
%   plots/sigma_k_vs_subhalo_factor.png     (existing placeholder; draft's
%       sigma_partition_vs_subhalo_factor.png does not exist)
%
% Open decisions flagged in blue:
%   - R_perp = 8.4 Mpc default: supervisor sign-off pending (2026-07-16
%     decision; internally ~4-12 Mpc defensible).
